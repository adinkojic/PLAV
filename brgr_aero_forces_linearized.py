...
def get_coeff(alpha, beta, Re, p, q ,r):
    alpha = np.clip(alpha, -15*math.pi/180, 15*math.pi/180)
...
def get_aero_forces(state):
    ...
    airspeed, beta, alpha = velocity_to_alpha_beta(np.array(state[3],state[4],state[5]))
    ...
    drag = C_L * qbar * Aref
    side = C_Z * qbar * Aref
    pitching_moment = C_M * qbar * Aref * mac
    yawing_moment   = C_Y * qbar * Aref #Coeff definition might be wrong
    rolling_moment  = C_R * qbar * Aref

    wind_to_stab = get_wind_to_stability_axis(alpha,beta)

    body_forces_wind = np.array([-drag, side, lift])
    body_forces_stab = quat.rotateVectorQ(wind_to_stab, body_forces_wind)

    moments = np.array([rolling_moment, -pitching_moment, yawing_moment])
